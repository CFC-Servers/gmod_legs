...
local localPlayerTable = localPlayer:GetTable()
hook.Add( "InitPostEntity", "GML:InitPostEntity", function()
    localPlayer = LocalPlayer()
    localPlayerTable = localPlayer:GetTable()
end )

local function getLegModel()
    if localPlayerTable.enforce_model then return localPlayerTable.enforce_model end
...
local function shouldDrawLegs()
    if areLegsEnabled then
        return ( localPlayer:Alive() or localPlayerTable.IsGhosted and localPlayer:IsGhosted() ) and not checkDrawVehicle() and GetViewEntity() == localPlayer and not localPlayer:ShouldDrawLocalPlayer() and not IsValid( localPlayer:GetObserverTarget() ) and not localPlayerTable.ShouldDisableLegs
    else
        return false
    end
end
